...
from deeplabv3.model_v3_TransposeConv import DeepLabV3Plus
import encoders
...
scheduler = lr_scheduler.StepLR(optimizer, step_size=130, gamma=0.5)

PATH = "./Corine/model/checkpoints/Corine_DSR_weights_TransposeConv_MSE_1_0_state_v3_0.pth"
...
def train_model(model, loss_func, optimizer, scheduler, start_epoch, train_loss, valid_loss, train_w_iou, train_m_iou, valid_w_iou, valid_m_iou, train_rmse, valid_rmse, num_epochs=25):
    ...
    for epoch in range(start_epoch, end_epoch):
        ...
        if valid_m_iou[-1] > best_iou:
            ...
            torch.save(state, "./Corine/model/checkpoints/Corine_DSR_weights_TransposeConv_MSE_1_0_state_v3_1.pth")
        print() # end of epoch
    ...
    ax1.legend()
    plt.close(fig1)
    fig1.savefig('./Corine/model/Plots/loss_weights_TransposeConv_MSE_1_0_state_v3_1.png')
    ...
    ax2.legend()
    plt.close(fig2)
    fig2.savefig('./Corine/model/Plots/w_iou_weights_TransposeConv_MSE_1_0_state_v3_1.png')
    ...
    ax3.legend()
    plt.close(fig3)
    fig3.savefig('./Corine/model/Plots/m_iou_weights_TransposeConv_MSE_1_0_state_v3_1.png')
    ...
    ax4.legend()
    plt.close(fig4)
    fig4.savefig('./Corine/model/Plots/rmse_weights_TransposeConv_MSE_1_0_state_v3_1.png')
    
    return model

model = train_model(model, loss_func, optimizer, scheduler, start_epoch, train_loss, valid_loss, train_w_iou, train_m_iou, valid_w_iou, valid_m_iou, train_rmse, valid_rmse, num_epochs=10)
torch.save(model.state_dict(), "./Corine/model/trained_models/Corine_DSR_weights_TransposeCon_MSE_1_0v_state_v3_1.pth")