...
from .decoder_PixelShuffle import DeepLabV3PlusDecoder

from base.model_v4 import SegmentationModel
from base.head import SegmentationHead
    ...
    def __init__(
       ...
       ###############################################################################################################################
        ...
        self.segmentation_head = SegmentationHead(
            ...
            upsampling_type='PixelShuffle')
        
        self.sssr = SSSR_Extra_Upsampling_Module(in_channels=self.sssr_decoder.out_channels, out_channels=classes)
        ...
        self.sisr = SISR_Extra_Upsampling_Module(in_channels=64, out_channels=4)
...
class SISR_Extra_Upsampling_Module(nn.Module):
    def __init__(self, in_channels, out_channels):
        super().__init__()
        
        self.up = Pixel_Shuffle_Module(in_channels, in_channels, 2)
        
        self.conv = nn.Sequential(
               nn.Conv2d(in_channels + 4, out_channels, kernel_size=3, stride=1, padding=1)
        )
    
    def forward(self, LR_sisr, x):
        # Upsample the LR_sisr features (output of the SISR SegmentationHead) x2, 64 channels
        
        y = self.up(LR_sisr)
                
        # Concatenate both feature maps (Skip connection with the Bicubic of the input image) -> 64 + 4 = 68 channels
        concat_features = torch.cat([y, x], dim=1)
        
        # Conv 3x3, 4 filters to fuse the feature maps to 4 channels
        HR_sisr = self.conv(concat_features)
        
        return HR_sisr
    
class SSSR_Extra_Upsampling_Module(nn.Module):
    def __init__(self, in_channels, out_channels):
        super().__init__()
        ...
        self.conv = nn.Sequential(
               nn.Conv2d(in_channels + 16, out_channels, kernel_size=3, stride=1, padding=1)
        )

        self.disperse = nn.Sequential(
            nn.Conv2d(4, 16, kernel_size=1, stride=1, padding=0)
        )
    
    def forward(self, LR_sssr, x):
        # Upsample the LR_sssr features (output of the SSSR SegmentationHead) x2, 128 channels
        
        y = self.up(LR_sssr)
        

        # Disperse the spectral information of the bicubic using a 1x1 conv with 16 filters
        x = self.disperse(x)

        # Concatenate both feature maps (Skip connection with the Bicubic of the input image) -> 128 + 4 = 132 channels
        concat_features = torch.cat([y, x], dim=1)
        
        # Conv 3x3, 4 filters to fuse the feature maps to 4 channels
        HR_sssr = self.conv(concat_features)
