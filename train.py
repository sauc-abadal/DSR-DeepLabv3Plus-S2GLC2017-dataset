...
print(device)
model.to(device)
iou = IoU(num_classes=15).to(device)
...
PATH = "./Corine/model/checkpoints/Corine_DSR_state_v2_1.pth"
...
def train_model(model, loss_func, optimizer, scheduler, start_epoch, train_loss, valid_loss, train_w_iou, train_m_iou, valid_w_iou, valid_m_iou, train_rmse, valid_rmse, num_epochs=25):
    ...
    for epoch in range(start_epoch, end_epoch):
        ...
        if valid_m_iou[-1] > best_iou:
            ...
            torch.save(state, "./Corine/model/checkpoints/Corine_DSR_state_v2_2.pth")
        print() # end of epoch
    ...
    ax1.legend()
    plt.close(fig1)
    fig1.savefig('./Corine/model/Plots/loss_state_v2_2.png')
    ...
    ax2.legend()
    plt.close(fig2)
    fig2.savefig('./Corine/model/Plots/w_ious_state_v2_2.png')
    ...
    ax3.legend()
    plt.close(fig3)
    fig3.savefig('./Corine/model/Plots/m_iou_state_v2_2.png')
    ...
    ax4.legend()
    plt.close(fig4)
    fig4.savefig('./Corine/model/Plots/rmse_state_v2_2.png')
...
torch.save(model.state_dict(), "./Corine/model/trained_models/Corine_DSR_state_v2_2.pth")