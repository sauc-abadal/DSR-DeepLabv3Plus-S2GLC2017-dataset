...
class SegmentationModel(torch.nn.Module):
    ...
    def forward(self, x):
        ...
        # - COMPUTE THE BICUBIC OF X -
        
        x_bi = F.interpolate(x, scale_factor=2, mode='bicubic', align_corners=False)
        ...
        #                                               -Upsample x2 -> shape (Bs, 128, H, W)
        # The Segmentation Head does the following:     -3x3 conv -> shape (Bs, 128, H, W)
        #                                               
        ...
        # The extra up-sampling module does the following:      
        #                                                       -Upsample x2 -> shape (Bs, num_classes, 2*H, 2*W)
        #                                                       -3x3 conv, num_classes filters
        
        sssr_HR = self.sssr(sssr_LR, x_bi) 
        ...
        #                                                 -Upsample x2 -> shape (Bs, 128, H, W)
        # The "Segmentation Head" does the following:     -3x3 conv -> shape (Bs, 64, H, W)
        #                                                 
        ...
        #                                                       -Concatenates with the bicubic of the input training image x
        #                                                       -3x3 conv, 4 filters
        
        
        #print(f'Bicubic of x: {x.shape}') # Should be (Bs, 3, 2*H, 2*W)
        
        # ----------------------------
        
        sisr_HR = self.sisr(sisr_LR, x_bi)
